module mod_pressure
    ...
    private
    public :: generate_laplacian_sparse, generate_laplacian_sparse_constant, calculate_pressure_sparse
...
subroutine calculate_pressure_sparse(A,PIN,R)
    ...
    do while ((ITER.lt.MAXITER).and.(ERROR.gt.TOLERANCE))
        ...
        ! P(imin-1,jmin-1:jmax+1) = -P(imin,jmin-1:jmax+1)
        ! P(imax+1,jmin-1:jmax+1) = -P(imax,jmin-1:jmax+1)
        ! Calculate Residual
...
subroutine generate_laplacian_sparse_constant(A,dx,dy)
    ...
    A(i,j,LEFT)     = 0                                        ! Right
    A(i,j,RIGHT)    = idx2                                     ! Left
    A(i,j,CENTER)   = A(i,j,CENTER) + (-3*idx2)                  ! Center
...
end subroutine generate_laplacian_sparse_constant
