...
%     contourf(xp,yp,umag,50,'edgecolor','none')
    ...
    for i = 1:2:3%:size(cilia,1) % No. of rows in the cilia file
        ciliaX = cilia(i,:); % Read x-coordinates
        ciliaY = cilia(i+1,:); % Read y-coordinates
        plot(ciliaX,ciliaY,'k-o','linewidth',3,'Markersize',5) 
    end
    % Horizontal links
    for i = 1%:4:size(cilia,1)
        ...
        % Horizontal links
        for j = 1:numel(px1)
            plot([px1(j),px2(j)],[py1(j),py2(j)],'k-o','linewidth',1,'Markersize',5) 
        end
        % Diagonal links: 1
        for j = 1:numel(px1)-1
            plot([px1(j),px2(j+1)],[py1(j),py2(j+1)],'k-o','linewidth',1,'Markersize',5)
        end
         % Diagonal links: 2
        for j = 1:numel(px1)-1
            plot([px1(j+1),px2(j)],[py1(j+1),py2(j)],'k-o','linewidth',1,'Markersize',5)
        end
    ...
    for i = 1:2:size(particle,1) % No. of rows in the particle file
        ...
        plot(particleX,particleY,'k-o','linewidth',3,'Markersize',5)
        % Join the ends
        plot([particleX(end), particleX(1)],[particleY(end), particleY(1)],'k-o','linewidth',3,'Markersize',5)
    end
   ...
   % Horizontal links
    for i = 1:4:size(particle,1) % No. of rows in the particle file
        ...
        for j = 1:numel(particleX1)
            plot([particleX1(j),particleX2(j)],[particleY1(j),particleY2(j)],'k-o','linewidth',1,'Markersize',5)
        end
        
        % Diagonal links: 1
        for j = 1:numel(particleX1)-1
            plot([particleX1(j),particleX2(j+1)],[particleY1(j),particleY2(j+1)],'k-o','linewidth',1,'Markersize',5)
        end
        % Join the ends
        plot([particleX1(end),particleX2(1)],[particleY1(end),particleY2(1)],'k-o','linewidth',1,'Markersize',5)
        
         % Diagonal links: 2
        for j = 1:numel(particleX1)-1
            plot([particleX1(j+1),particleX2(j)],[particleY1(j+1),particleY2(j)],'k-o','linewidth',1,'Markersize',5)
        end
        % Join the ends
        plot([particleX1(1),particleX2(end)],[particleY1(1),particleY2(end)],'k-o','linewidth',1,'Markersize',5)
        
    end
    axis equal
    title('Cilia and Particle')
    xlim([0.6 2])
    ylim([0 1])
%     %% Plot mesh
...
%% Plot tip deflection
    ...
    cilia = load(ciliaFile(iFile).name);
    for i = 1%:size(cilia,1) % No. of rows in the cilia file
        ciliaY1 = cilia(i+1,end); % Read the tip deflection of the first layer
        ciliaY2 = cilia(i+3,end); % Read y-coordinates
        plot(iFile,(ciliaY1+ciliaY2)/2,'rx') 
    end
    ylim([0.22 0.32])
    xlabel('Time steps')
